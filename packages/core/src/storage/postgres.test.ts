...
const PG_URL = process.env.SISU_TEST_PG;
...
async function truncateAll(): Promise<void> {
  await pool.query(
    'TRUNCATE work_items, execution_plans, plan_steps, mail, leases RESTART IDENTITY CASCADE',
  );
}

describe.skipIf(!PG_URL)('PostgresStorage — WorkItems', () => {
...
describe.skipIf(!PG_URL)('PostgresStorage — ExecutionPlans', () => {
  let storage: PostgresStorage;
...
describe.skipIf(!PG_URL)('PostgresStorage — Mail', () => {
  let storage: PostgresStorage;
...
describe.skipIf(!PG_URL)('PostgresStorage — Leases', () => {
  let storage: PostgresStorage;
...
describe.skipIf(!PG_URL)('PostgresStorage — Roles and Workflows (in-memory)', () => {
  const mockRoles = [
    {
      id: 'test-role',
      name: 'Test Role',
  ...
  beforeEach(async () => {
    await truncateAll();
    storage = new PostgresStorage(pool, { roles: mockRoles, workflows: mockWorkflows });
  });
